...
tic;
%set b to something reasonably small so that as we vary 's' running times
%are reasonably quick. This blocksize applies for all experiments.
b =16;
s = 1;
...
tic;
% s1 = 10, s2 = 100, s3 = 200. Might want to leave this alone. 
%Decrease values if running time is too long.

s1 = 10;
results_1 = cabdcd_conv(X, y, lambda, s1, b, maxit, tol, 1, freq, w);
toc;
...
tic;
s2 = 100;
results_2 = cabdcd_conv(X, y, lambda, s2, b, maxit, tol, 1, freq, w);
toc;
...
tic;
s3 = 200;
results_3 = cabdcd_conv(X, y, lambda, s3, b, maxit, tol, 1, freq, w);
toc;
...
leg = legend(strcat('BDCD s =  ', int2str(s)), strcat('BDCD s =  ', int2str(s1)),...
    strcat('BDCD s =  ', int2str(s2)), strcat('BDCD s= ', int2str(s3)), '\epsilon_{mach}');
set(leg, 'FontSize', 18);
set(gca, 'FontSize', 16);
ylim([-17 5]);
...
h = boxplot(log10([results.gramcond(2:end)', results_1.gramcond(2:end)', results_2.gramcond(2:end)', results_3.gramcond(2:end)']),'Label',... 
    { strcat('BDCD s = ', int2str(b)),...
    strcat('BDCD s = ', int2str(s1)),...
    strcat('BDCD s = ', int2str(s2)),...
    strcat('BDCD s = ', int2str(s3))}, 'Colors', 'k');
