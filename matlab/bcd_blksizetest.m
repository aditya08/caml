...
tic;
%b = 1, is coordinate descent. Do not change.
...
% change b1 as needed.
b1 = 2;
results_1 = cabcd_conv(X', y, lambda, s, b1, maxit, tol, 1, freq, w);
toc;
...
% change b2 as needed.
s = 1;
results_2 = cabcd_conv(X', y, lambda, s, b2, maxit, tol, 1, freq, w);
toc;
...
b3 = 6;
% change b1 as needed.
results_3 = cabcd_conv(X', y, lambda, s, b3, maxit, tol, 1, freq, w);
toc;
...
leg = legend(strcat('BCD b = ', int2str(b)), strcat('BCD b = ', int2str(b1)),...
    strcat('BCD b = ', int2str(b2)), strcat('BCD b = ', int2str(b3)), '\epsilon_{mach}');
set(leg, 'FontSize', 18);
set(gca, 'FontSize', 16);
ylim([-17 5]);
...
grid on;
% 
plot(log10(results.iters*n + 1), log10((results.obj - objopt)/objopt  + eps), '-k', 'LineWidth', 2, 'MarkerSize', 10)
plot(log10(results_1.iters*(b1^2*n + b1^3)+1), log10((results_1.obj - objopt)/objopt  + eps), 'o--k', 'LineWidth', 2, 'MarkerSize', 10)
plot(log10(results_2.iters*(b2^2*n + b2^3)+1), log10((results_2.obj - objopt)/objopt  + eps), '+--k', 'LineWidth', 2, 'MarkerSize', 10)
plot(log10(results_3.iters*(b3^2*n + b3^3)+1), log10((results_3.obj - objopt)/objopt  + eps), 'x--k', 'LineWidth', 2, 'MarkerSize', 10)
H = refline(0,log10(eps));
H.Color = 'k';
H.LineWidth = 2;
H.MarkerSize = 10;
