function [ results ] = cabdcd_conv(X, y, lambda,s, b, maxit, tol, seed, freq, wopt)
...
%     alpha(index) = alpha(index) + dalsamp';
    ...
    if(norm(out_nrm) <= tol)
        ...
        if(norm(-lambda*w-((1/n)*X*(X'*w)) + ((1/n)*X*y)) <= tol)
            ...
            disp(strcat(string('Converged to solution with dual estimated residual: '),num2str(norm(out_nrm))));
            return;
            %out_nrm = zeros(m,1);
        end
    end
